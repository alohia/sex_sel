function [ Hky,ks,CYy,cYy,CXx,cXx,ky,vyy,vxx,i,dy,S ] = solve_model_girls( X, Y, params, tr )
...
while(1)
    vin = vxx(n,i-1);
    temp = @(vi) real(set_vin_girls(n,H,al,vi,X,Y,i,vxx,tr));
    ...
    [~,xend,vxx,Hky,ks,CYy,cYy,CXx,cXx,ky,vyy,dy,S ] = set_vin_girls(n,H,al,vinit,X,Y,i,vxx,tr);
    while (xend ~= 0)
        vxx(xend,i) = S(xend,1);
